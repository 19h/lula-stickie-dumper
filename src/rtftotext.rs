...
pub fn make_input_reader(infile: Option<&str>) -> Result<io::BufReader<Box<dyn io::Read>>> {
...
pub fn make_output_writer(outfile: Option<&str>) -> Result<io::BufWriter<Box<dyn io::Write>>> {
...
pub fn convert(infile: Option<&str>, outfile: Option<&str>) -> Result<()> {
    ...
    if let Some(tokens) = tokenize(reader) {
        if let Err(e) = write_plaintext(&tokens, writer) {
            warn!("Error writing to output: {}", e);
        }
    }
...
pub fn tokenize<R: Read>(mut reader: R) -> Option<Vec<Token>> {
